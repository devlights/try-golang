...
func aboutGo113Error() {
	...
	//
	// ただし、errors や golang.org/x/xerrors に存在するスタックトレース付きの書式化文字列はサポートされていない。
	// (: %v とか : %w とか %+v とか %+w とか)
	// ------------------------------------------------------
	// 1.13 以降 (1)
...
func aboutGoError() {
	...
	// 少し難があるため、 errors (https://github.com/errors) を利用している人も多い。
	// しかし、Go2に向けてエラー処理も使いやすいように変化しているので、今は過渡期と言えるかもしれない。
	// 実際、Go 1.13 で追加された errors.Unwrap や Is, As, %w などは、元々 errors に存在している機能である。
	//
